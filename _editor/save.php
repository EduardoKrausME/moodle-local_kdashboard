...
if ($action) {
    ...
    switch ($action) {
        case "delete":
            ...
            preg_match('/pluginfile.php\/(\d+)\/local_kdashboard\/(\w+)\/(\d+)\/(.*)/', $data->file, $filePartes);
        ...
        case "save":
            if ($page == "webpages") {
                ...
                $events = $DB->get_record("local_kdashboard_event", ["id" => $id]);
                $events->message = $html;
                $DB->update_record("local_kdashboard_event", $events);
            } else if ($page == "settings") {
                set_config($id, $html, "local_kdashboard");
            }
            echo json_encode([
                "success" => 1,
