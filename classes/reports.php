...
class reports {
    ...
    public function dashboard() {
        ...
        $koperereportcats = $DB->get_records("local_kdashboard_rcat");

        /** @var local_kdashboard_rcat $koperereportcat */
        foreach ($koperereportcats as $koperereportcat) {
            ...
            $koperereportss = $DB->get_records("local_kdashboard_reprt",
                ["reportcatid" => $koperereportcat->id]);

            /** @var local_kdashboard_reprt $koperereports */
            foreach ($koperereportss as $koperereports) {
                ...
                $url = local_kdashboard_makeurl("reports", "load_report", ["report" => $koperereports->id]);
    ...
    public function load_report() {
        ...
        /** @var local_kdashboard_reprt $koperereports */
        $koperereports = $DB->get_record("local_kdashboard_reprt",
            ["id" => $report]);
        header::notfound_null($koperereports, get_string_kopere("reports_notfound"));

        /** @var local_kdashboard_rcat $koperereportcat */
        $koperereportcat = $DB->get_record("local_kdashboard_rcat",
        ...
        dashboard_util::add_breadcrumb(get_string_kopere("reports_title"),
            local_kdashboard_makeurl("reports", "dashboard"));
        dashboard_util::add_breadcrumb($titlecat,
            local_kdashboard_makeurl("reports", "dashboard", ["type" => $koperereportcat->type]));
        ...
        if (strlen($koperereports->prerequisit) && $courseid == 0) {
            ...
            if (strpos($koperereports->reportsql, "local_kdashboard") === 0) {
                ...
                $url = local_kdashboard_makeurl("reports", "getdata", ["report" => $report, "courseid" => $courseid]);
                ...
                $urldow = local_kdashboard_makeurl("reports", "download", ["report" => $report, "courseid" => $courseid]);
    ...
    private function prerequisit($report, $pre) {
        if ($pre == "listCourses") {
            ...
            $table->set_click_redirect(
                local_kdashboard_makeurl("reports", "load_report",
    ...
    public function getdata() {
        ...
        /** @var local_kdashboard_reprt $koperereports */
        $koperereports = $DB->get_record("local_kdashboard_reprt", ["id" => $report]);
    ...
    public static function global_menus() {
        ...
        $koperereportcats = $DB->get_records("local_kdashboard_rcat", ["enable" => 1]);
        /** @var local_kdashboard_rcat $koperereportcat */
        foreach ($koperereportcats as $koperereportcat) {
            // Executa o SQL e vrifica se o SQL retorna status>0.
            if (strlen($koperereportcat->enablesql)) {
