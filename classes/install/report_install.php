...
use local_kdashboard\report\user_field;
use local_kdashboard\vo\local_kdashboard_rcat;
use local_kdashboard\vo\local_kdashboard_reprt;
...
class report_install {
    ...
    public static function create_categores() {
        ...
        $reportcat = local_kdashboard_rcat::create_by_default();
    ...
    public static function create_reports() {
        ...
        $report = local_kdashboard_reprt::create_by_default();
        $report->reportcatid = $DB->get_field("local_kdashboard_rcat", "id", ["type" => "badge"]);
...
SELECT id, name, description, type, status,
  ...
  FROM {badge} b";
        ...
        $report->foreach = "\\local_kdashboard\\report\\report_foreach::badge_status_text";
...
ORDER BY u.username";
        ...
        $report->foreach = "\\local_kdashboard\\report\\report_foreach::badge_criteria_type";
        $report->columns = json_encode(["columns" => $report->columns]);
        self::report_insert($report);

        // Novo report.
        $report = local_kdashboard_reprt::create_by_default();
        $report->reportcatid = $DB->get_field("local_kdashboard_rcat", "id", ["type" => "courses"]);
 ...
 LEFT JOIN (
  ...
  GROUP BY u.id";
        ...
        $report->foreach = "\\local_kdashboard\\report\\report_foreach::userfullname";
        $report->columns = json_encode(["columns" => $report->columns]);
        if ($CFG->dbtype != "pgsql") {
            self::report_insert($report);
        }
...
SELECT concat(c.id,g.id), c.id, c.fullname, c.shortname, g.name, c.visible, c.groupmode
 ...
 WHERE c.groupmode > 0";
        ...
        $report->foreach = "\\local_kdashboard\\report\\report_foreach::courses_group_mode";
        $report->columns = json_encode(["columns" => $report->columns]);
        self::report_insert($report);
        ...
        $report->prerequisit = "listCourses";
        $report->reportsql = '\\local_kdashboard\\report\\custom\\course_access';
        $report->columns = "";
        self::report_insert($report);
        ...
        $report->prerequisit = "listCourses";
        $report->reportsql = '\\local_kdashboard\\report\\custom\\course_access_grade';
        $report->columns = "";
        self::report_insert($report);
        ...
        $report->prerequisit = "listCourses";
        $report->reportsql = '\\local_kdashboard\\report\\custom\\course_last_access';
        $report->columns = "";
        self::report_insert($report);

        // Novo report.
        $report = local_kdashboard_reprt::create_by_default();
        $report->reportcatid = $DB->get_field("local_kdashboard_rcat", "id", ["type" => "enrol_cohort"]);
...
ORDER BY u.firstname";
        ...
        // Novo report.
        $report = local_kdashboard_reprt::create_by_default();
        $report->reportcatid = $DB->get_field("local_kdashboard_rcat", "id", ["type" => "enrol_guest"]);
...
SELECT CONCAT(lsl.userid, lsl.timecreated), {$alternatenames}, u.id AS userid, lsl.timecreated, lsl.ip
  FROM {local_kdashboard_login} lsl
 ...
 WHERE u.id = 1";
        ...
        // Novo report.
        $report = local_kdashboard_reprt::create_by_default();
        $report->reportcatid = $DB->get_field("local_kdashboard_rcat", "id", ["type" => "server"]);
...
SELECT c.id, c.fullname, c.shortname, c.visible, c.timecreated,
 ...
 WHERE c.id > 1";
        ...
        // Novo report.
        $report = local_kdashboard_reprt::create_by_default();
        $report->reportcatid = $DB->get_field("local_kdashboard_rcat", "id", ["type" => "user"]);
...
SELECT ue.id, u.id AS userid, ul.timeaccess, {$alternatenames}, u.email, u.city, u.lastaccess,
 ...
 WHERE ul.timeaccess IS NULL";
    ...
    private static function report_cat_insert($reportcat) {
        ...
        $koperereportcatexist = $DB->record_exists("local_kdashboard_rcat", ["type" => $reportcat->type]);
        if (!$koperereportcatexist) {
            $DB->insert_record("local_kdashboard_rcat", $reportcat);
        }
    }
    ...
    private static function report_insert($report) {
        ...
        $koperereports = $DB->get_record("local_kdashboard_reprt", ["reportkey" => $report->reportkey]);
        if (!$koperereports) {
            $DB->insert_record("local_kdashboard_reprt", $report);
        }
    }
}
