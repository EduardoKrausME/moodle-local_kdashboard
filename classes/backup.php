...
class backup {
    ...
    public function dashboard() {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === "WIN") {
            ...
            if (server_util::function_enable("shell_exec")) {
                ...
                button::add(get_string_kopere("backup_newnow"), local_kdashboard_makeurl("backup", "execute"));
                button::add(get_string_kopere("backup_newsqlnow"), local_kdashboard_makeurl("backup", "execute_dumpsql"));
            ...
            foreach ($backups as $backup) {
                ...
                $backupslista[] = [
                    ...
                    "acoes" =>
                        ...
                        button::icon("download", local_kdashboard_makeurl("backup", "download", ["file" => $p[0]])) .
                        "&nbsp;&nbsp;&nbsp; " .
                        button::icon_popup_table("delete", local_kdashboard_makeurl("backup", "delete", ["file" => $p[0]])) .
                        "</div>",
                ];
            }
    ...
    public function execute() {
        ...
        if (!server_util::function_enable("shell_exec")) {
            ...
            header::location(local_kdashboard_makeurl("backup", "dashboard"));
        }
    ...
    public function execute_dumpsql() {
        ...
        dashboard_util::add_breadcrumb(get_string_kopere("backup_title"), local_kdashboard_makeurl("backup", "dashboard"));
        ...
        foreach ($tables as $table => $val) {
            ...
            $PAGE->requires->js_call_amd("local_kdashboard/backup", "backup_animate_scrollTop", ["#tabela-dump-$table"]);
        ...
        button::add(get_string_kopere("backup_returnlist"), local_kdashboard_makeurl("backup", "dashboard"));
        echo "</div>";

        $PAGE->requires->js_call_amd("local_kdashboard/backup", "backup_animate_scrollTop", ["#end-page-to"]);
    ...
    public function delete() {
        ...
        if (file_exists($backupfile)) {
            if ($status) {
                ...
                dashboard_util::add_breadcrumb(get_string_kopere("backup_title"),
                    local_kdashboard_makeurl("backup", "dashboard"));
                ...
                button::delete(get_string("yes"),
                    local_kdashboard_makeurl("backup", "delete", ["file" => $file, "status" => "sim"]), "", false);
                button::add(get_string("no"),
                    local_kdashboard_makeurl("backup", "dashboard"), "margin-left-10", false);
                echo "    </div>
                      </div>";
